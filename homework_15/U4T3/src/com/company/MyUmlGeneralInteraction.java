...
public class MyUmlGeneralInteraction implements UmlGeneralInteraction {
    ...
    private List<UmlAttribute> umlAttributesClass = new ArrayList<>();
    ...
    public MyUmlGeneralInteraction(UmlElement... elements) {
        ...
        for (Map.Entry<String, UmlAttribute> entry : umlAttributes.entrySet()) {
            if (umlClassHashMap.containsKey(entry.getValue().getParentId())) {
                ...
                umlAttributesClass.add(entry.getValue());
            } else if (umlInterfaceHashMap.containsKey(entry.getValue().getParentId())) {
                umlInterfaceHashMap.get(entry.getValue().getParentId()).getUmlAttributes()
                    .add(entry.getValue());
                umlAttributesClass.add(entry.getValue());
            } else if (umlCollaborationHashMap.containsKey(entry.getValue().getParentId())) {
                umlCollaborationHashMap.get(entry.getValue().getParentId()).getUmlAttributes()
                    .add(entry.getValue());
            }
        }
        ...
        for (UmlAssociation umlAssociation : umlAssociations) {
            ...
            MyInterface umlInterface1 = umlInterfaceHashMap.get(umlAssociationEnd1.getReference());
            MyInterface umlInterface2 = umlInterfaceHashMap.get(umlAssociationEnd2.getReference());
            if (umlClass1 != null && umlClass2 != null) {
                if (umlClass1.equals(umlClass2)) {
                    umlClass1.addUmlClass(umlClass2);
                    umlClass1.getUmlAssociationEnds().add(umlAssociationEnd2);
                    umlClass1.getUmlAssociationEnds().add(umlAssociationEnd1);
                } else {
                    umlClass1.addUmlClass(umlClass2);
                    umlClass1.getUmlAssociationEnds().add(umlAssociationEnd2);
                    umlClass2.addUmlClass(umlClass1);
                    umlClass2.getUmlAssociationEnds().add(umlAssociationEnd1);
                }
            } else if (umlClass1 != null && umlInterface2 != null) {
                umlClass1.getUmlInterfacesAssociate().add(umlInterface2);
                umlClass1.getUmlAssociationEnds().add(umlAssociationEnd2);
            } else if (umlClass2 != null && umlInterface1 != null) {
                umlClass2.getUmlInterfacesAssociate().add(umlInterface1);
                umlClass2.getUmlAssociationEnds().add(umlAssociationEnd1);
            }
        }
        ...
        for (Map.Entry<String, MyUmlLifeline> entry : umlLifelineHashMap.entrySet()) {
            ...
            if (umlAttributes.containsKey(entry.getValue().getUmlLifeline().getRepresent())) {
    ...
    public void checkForUml001() throws UmlRule001Exception {
        Set<AttributeClassInformation> attributeClassInformations = new HashSet<>();
        for (Map.Entry<String, MyUmlClass> item : umlClassHashMap.entrySet()) {
            attributeClassInformations.addAll(item.getValue().R001());
        }
        if (!attributeClassInformations.isEmpty()) {
            throw new UmlRule001Exception(attributeClassInformations);
        }
    }

    @Override
    public void checkForUml002() throws UmlRule002Exception {
        Set<MyUmlClassorInterface> umlClassOrInterfaces = new HashSet<>();
        for (Map.Entry<String, MyUmlClass> item : umlClassHashMap.entrySet()) {
            item.getValue().clear();
            if (item.getValue().R002(item.getValue())) {
                MyUmlClassorInterface umlClassorInterface =
                    new MyUmlClassorInterface(item.getValue());
                umlClassOrInterfaces.add(umlClassorInterface);
            }
        }
        for (Map.Entry<String, MyInterface> item : umlInterfaceHashMap.entrySet()) {
            if (item.getValue().R002(item.getValue())) {
                MyUmlClassorInterface umlClassorInterface =
                    new MyUmlClassorInterface(item.getValue());
                umlClassOrInterfaces.add(umlClassorInterface);
        ...
        if (!umlClassOrInterfaces.isEmpty()) {
            throw new UmlRule002Exception(umlClassOrInterfaces);
        }
    }
    ...
    public void checkForUml003() throws UmlRule003Exception {
        ...
        if (!umlClassOrInterfaces.isEmpty()) {
            throw new UmlRule003Exception(umlClassOrInterfaces);
        }
    }
    ...
    public void checkForUml004() throws UmlRule004Exception {
        ...
        for (Map.Entry<String, MyUmlClass> item : umlClassHashMap.entrySet()) {
            List<MyInterface> myInterfaces = new ArrayList<>();
            if (item.getValue().R004(myInterfaces)) {
                umlClasses.add(item.getValue().getUmlClass());
            }
        }
        if (!umlClasses.isEmpty()) {
            throw new UmlRule004Exception(umlClasses);
        }
    }

    @Override
    public void checkForUml005() throws UmlRule005Exception {
        for (UmlAttribute item : umlAttributesClass) {
            if (item.getName() == null) {
                throw new UmlRule005Exception();
            }
        }
        for (UmlParameter item : umlParameters) {
            if (item.getDirection() != Direction.RETURN && item.getName() == null) {
                throw new UmlRule005Exception();
            }
        }
        for (Map.Entry<String, MyOperation> item : umlOperations.entrySet()) {
            if (item.getValue().getUmlOperation().getName() == null) {
                throw new UmlRule005Exception();
            }
        }
        for (Map.Entry<String, MyUmlClass> item : umlClassHashMap.entrySet()) {
            if (item.getValue().getUmlClass().getName() == null) {
                throw new UmlRule005Exception();
            }
        }
        for (Map.Entry<String, MyInterface> item : umlInterfaceHashMap.entrySet()) {
            if (item.getValue().getUmlInterface().getName() == null) {
                throw new UmlRule005Exception();
            }
        }
    }
    ...
    public void checkForUml007() throws UmlRule007Exception {
        for (Map.Entry<String, MyUmlLifeline> item : umlLifelineHashMap.entrySet()) {
            String id = item.getValue().getUmlLifeline().getParentId();
            if (item.getValue().getUmlAttribute() == null) {
                throw new UmlRule007Exception();
            } else if (!umlInteractionHashMap.get(id).getUmlInteraction().getParentId()
                .equals(item.getValue().getUmlAttribute().getParentId())) {
                throw new UmlRule007Exception();
            }
        }
    }
    ...
    public void checkForUml008() throws UmlRule008Exception {
        for (Map.Entry<String, MyRegion> item : regionHashMap.entrySet()) {
            if (item.getValue().R008()) {
